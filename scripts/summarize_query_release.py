...
def main():
    ...
    documents = {}
    sources = {}
    for name in ('capacity', 'engines', 'boundaries', 'omero', 'browser', 'vm_promotions', 'vm_cache'):
        path = getattr(args, name)
        raw = path.read_bytes()
        documents[name] = [json.loads(line) for line in raw.decode('utf-8-sig').splitlines() if line.strip()]
        sources[name] = {'file': path.name, 'sha256': hashlib.sha256(raw).hexdigest()}
    ...
    for key, count in (('boundaries', 4), ('omero', 3), ('browser', 3), ('vm_promotions', 10), ('vm_cache', 6)):
    ...
    report = {'schema': 'analysis-query-release-evidence-v1', 'date': '2026-09-08', 'status': 'passed',
              'evidence': sources, 'capacity_configuration': next(r for r in capacity if r.get('event') == 'configuration'),
